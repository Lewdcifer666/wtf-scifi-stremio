...
Replace `OWNER/REPO` with the actual GitHub repository before creating the scheduled task.

```text
Read the current public GitHub repository OWNER/REPO before making any recommendation decisions. Read data/taste-profile.json, data/library.json, data/rejections.json, and data/discovery-log.json.
...
Commit the updated source files directly to the default branch of OWNER/REPO with a concise commit message such as "Daily discovery: 3 movies, 1 series — 2026-08-23". The repository's GitHub Actions workflows will resolve metadata, rebuild the Stremio catalogs, and deploy GitHub Pages automatically.
